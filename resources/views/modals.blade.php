...
@if (Request::is('projects') ||
    ...
    <div class="modal fade" id="create_status_modal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <form method="POST" class="modal-content form-submit-event" action="{{ url('status/store') }}">
                @csrf
                    ...
                    <div class="row">
                        <div class="mb-3">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="link_to_project" name="link_to_project" value="1" checked>
                                <label class="form-check-label" for="link_to_project">
                                    <?= get_label('linked_to_project', 'Linked to a project') ?>
                                </label>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-12 mb-3">
                            <select name="workspace_id" id="workspace_id" class="form-control">
                                @foreach ($workspaces as $workspace)
                                    <option value="{{ $workspace->id }}" {{ $workspace->id == getWorkspaceId() ? 'selected' : '' }}>{{ $workspace->title }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
            ...
            <form action="{{ url('tasks/store') }}" class="form-submit-event modal-content" method="POST">
                ...
                <div class="modal-body">
                            ...
                            <select class="form-select statusDropdown" name="status_id">
                                @isset($statuses)
                                    @foreach ($statuses as $status)
                                        <option value="{{ $status->id }}" data-color="{{ $status->color ?? 'secondary' }}"
                            ...
                            <select class="form-select priorityDropdown" name="priority_id"
                                ...
                                @isset($priorities)
                                    @foreach ($priorities as $priority)
                                        <option value="{{ $priority->id }}" data-color="{{ $priority->color ?? 'secondary' }}">
                    ...
                    <div class="row">
                        <div class="mb-3">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="link_to_project" name="link_to_project" value="1" {{ isset($project->id) ? 'checked' : '' }}>
                                <label class="form-check-label" for="link_to_project">
                                    <?= get_label('linked_to_project', 'Linked to a project') ?>
                                </label>
                            </div>
                        ...
                        if (!isset($project->id)) {
                                                                                                                    ?>
                        <div class="mb-3" id="project_select_wrapper">
                            <label class="form-label"
                                for="user_id"><?= get_label('select_project', 'Select project') ?>
                                <small class="text-muted ms-1"><?= get_label('optional', 'Optional') ?></small>
                            </label>
                            <select class="form-control selectTaskProject projects_select" name="project"
                                data-placeholder="<?= get_label('type_to_search', 'Type to search') ?>"
                                data-single-select="true" data-allow-clear="true">
                            </select>
                        </div>
                        <?php } else {
                    ...
                    <div class="row" id="selectTaskUsers">
                        <div class="mb-3" id="project_users_wrapper">
                            <label class="form-label"
                                for="user_id_project"><strong><?= get_label('project_users', 'Project users') ?></strong> <span
                                ...
                                <b>{{ $project->title }}</b>)
                                <?php } ?></label>
                            <!-- Project-scoped users selector (original behavior) -->
                            <select class="form-control" id="user_id_project" name="user_id[]" multiple="multiple"
                                data-placeholder="<?= get_label('type_to_search', 'Type to search') ?>">
                            </select>
                        </div>
                        <div class="mb-3" id="workspace_users_wrapper">
                            <label class="form-label" for="user_id_any">
                                <strong><?= get_label('workspace_users', 'Workspace users') ?></strong>
                                <small class="text-muted"><?= get_label('optional', 'Optional') ?></small>
                            </label>
                            <!-- Workspace-wide users selector (AJAX) -->
                            <select class="form-control users_select" id="user_id_any" name="users_id[]" multiple="multiple"
                                data-placeholder="<?= get_label('type_to_search', 'Type to search') ?>" data-consider-workspace="true">
                                    @isset($users)
                    ...
                    <div class="row" id="task_list_wrapper">
                        <div class="mb-3">
        ...
        <div class="modal-dialog modal-lg" role="document">
            <form action="{{ url('tasks/update') }}" class="form-submit-event modal-content" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-12 mb-3" id="project_select_wrapper">
                            <label for="project_title" class="form-label"><?= get_label('project', 'Project') ?>
                                <span class="asterisk">*</span></label>
                    ...
                    @endif
                    <div class="mb-3" id="task_list_wrapper">
    ...
    <div class="modal fade" id="create_project_modal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <form action="{{ url('projects/store') }}" class="form-submit-event modal-content" method="POST">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel1">
                        <?= get_label('créer une diligence', 'Créer une diligence') ?></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                @csrf
                    ...
                    <div class="row">
                        <div class="mb-3">
                            ...
                            <select class="form-control users_select" name="user_id[]" multiple="multiple"
                                data-placeholder="<?= get_label('type_to_search', 'Type to search') ?>" data-consider-workspace="false">
                                    @isset($users)
                                        @foreach($users as $user)
                                            <option value="{{ $user->id }}">{{ $user->first_name }} {{ $user->last_name }}</option>
                                        @endforeach
                                ...
                                data-placeholder="<?= get_label('type_to_search', 'Type to search') ?>" data-consider-workspace="false">
                                    @isset($clients)
                                        @foreach($clients as $client)
                                            <option value="{{ $client->id }}">{{ $client->company ?: ($client->first_name.' '.$client->last_name) }}</option>
                                        @endforeach
                                    @endisset
                            </select>
                        </div>
