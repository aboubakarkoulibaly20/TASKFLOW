    ...
    <div class="card-body card-body-task-draggable">
                ...
                <ul class="list-unstyled users-list m-0 avatar-group d-flex align-items-center">
                    ...
                    if ($count > 0) {
                        foreach ($clients as $client) {
                            if ($displayed < 10) { ?>
                                <li class="avatar avatar-sm pull-up" title="<?= $client->company ?: ($client->first_name . ' ' . $client->last_name) ?>">
                                    <a href="{{ url('/clients/profile/' . $client->id) }}">
                                        <img src="<?= $client->photo ? asset('storage/' . $client->photo) : asset('storage/photos/no-image.jpg') ?>" class="rounded-circle" alt="<?= $client->company ?: ($client->first_name . ' ' . $client->last_name) ?>">
                                    </a>
                                </li>
                            <?php
                                $displayed++;
        ...
        <div class="d-flex flex-column">
            ...
            <div class="d-flex align-items-center mb-3">
                <!-- Status select -->
                <select class="form-select form-select-sm select-bg-label-{{ $task->status->color ?? 'secondary' }}" id="statusSelect" data-id="{{ $task->id }}" data-original-status-id="{{ $task->status->id }}" data-original-color-class="select-bg-label-{{ $task->status->color ?? 'secondary' }}" data-type="task" data-reload="true">
                    @foreach($statuses as $status)
                    @php
                    $disabled = canSetStatus($status) ? '' : 'disabled';
                    @endphp
                    <option value="{{ $status->id }}" class="badge bg-label-{{ $status->color ?? 'secondary' }}" {{ $task->status->id == $status->id ? 'selected' : '' }} {{ $disabled }}>
                        {{ $status->title }}
                    </option>
                    @endforeach
                </select>
            ...
            <select class="form-select form-select-sm select-bg-label-{{$task->priority ? $task->priority->color : 'secondary'}} mb-3" id="prioritySelect" data-id="{{ $task->id }}" data-original-priority-id="{{$task->priority ? $task->priority->id : ''}}" data-original-color-class="select-bg-label-{{$task->priority ? $task->priority->color : 'secondary'}}" data-type="task">
                ...
                <option value="{{ $priority->id }}" class="badge bg-label-{{ $priority->color ?? 'secondary' }}" {{ $task->priority && $task->priority->id == $priority->id ? 'selected' : '' }}>
                    {{$priority->title}}
                </option>
                @endforeach
            </select>
