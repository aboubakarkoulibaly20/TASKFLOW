...
$(document).on("click", ".edit-status", function () {
    var id = $(this).data("id");
    var $modal = $("#edit_status_modal");
    $modal.modal("show");

    var $statusColor = $modal.find("#status_color");
    var classes = $statusColor.attr("class").split(" ");
    var currentColorClass = classes.filter(function (className) {
        return className.startsWith("select-");
    })[0];
    ...
    $.ajax({
        ...
        dataType: "json",
        success: function (response) {
            // Always scope to the edit modal
            $modal.find("#status_id").val(response.status.id);              // name="id" gets set
            $modal.find("#status_title").val(response.status.title);
            $statusColor
                .val(response.status.color)
                .removeClass(currentColorClass)
                .addClass("select-bg-label-" + response.status.color);

            // If you want to preselect workspace in edit (optional)
            if (response.status.workspace_id) {
                $modal.find("#workspace_id").val(response.status.workspace_id).trigger("change");
            }

            var usersSelect = $modal.find('.js-example-basic-multiple[name="role_ids[]"]');
            usersSelect.val(response.roles).trigger("change");
        },
    });
});
...
// ==========================
function toggleProjectLinkedUI($modal) {
    var linked = $modal.find('#link_to_project').is(':checked');
    var $projectWrapper = $modal.find('#project_select_wrapper');
    var $projectUsersWrapper = $modal.find('#project_users_wrapper');
    var $taskListWrapper = $modal.find('#task_list_wrapper');
    var $workspaceUsersWrapper = $modal.find('#workspace_users_wrapper');
    $projectWrapper.toggle(linked);
    $projectUsersWrapper.toggle(linked);
    // Masquer la task list si non lié à un projet
    if ($taskListWrapper.length) {
        $taskListWrapper.toggle(linked);
    }
    // Workspace users visible uniquement si non lié à un projet
    if ($workspaceUsersWrapper.length) {
        $workspaceUsersWrapper.toggle(!linked);
    }
    if (!linked) {
        // Vider la sélection du projet
        var $projectSelect = $modal.find('.selectTaskProject');
        if ($projectSelect.length) {
            $projectSelect.val(null).trigger('change');
        }
        // Vider les utilisateurs liés au projet
        $modal.find('#user_id_project').val(null).trigger('change');
        // Vider la task list
        $modal.find('#task_list').val(null).trigger('change');
    }
}

// Synchroniser à l'ouverture du modal
$(document).on('shown.bs.modal', '#create_task_modal', function () {
    var $modal = $('#create_task_modal');
    toggleProjectLinkedUI($modal);
});

// Réagir au changement du switch
$(document).on('change', '#link_to_project', function () {
    var $modal = $('#create_task_modal');
    toggleProjectLinkedUI($modal);
});

// Synchroniser à l'ouverture du modal d'édition
$(document).on('shown.bs.modal', '#edit_task_modal', function () {
    var $modal = $('#edit_task_modal');
    toggleProjectLinkedUI($modal);
});

// Réagir au changement du switch dans le modal d'édition
$(document).on('change', '#edit_task_modal #link_to_project', function () {
    var $modal = $('#edit_task_modal');
    toggleProjectLinkedUI($modal);
});
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#edit_completion_percentage")
                ...
                .trigger("change");
            // Determine if task is linked to a project
            var hasProject = !!(response.project && response.project.id);
            var $editModal = $("#edit_task_modal");
            $editModal.find('#link_to_project').prop('checked', hasProject).trigger('change');

            // Project-scoped users select
            var projectUsersSelect = $editModal.find('#user_id_project');
            projectUsersSelect.empty();
            if (hasProject && response.project.users && response.project.users.length > 0) {
                response.project.users.forEach(function (user) {
                    ...
                    projectUsersSelect.append(userOption);
                });
            }
            // Workspace-wide users select
            var anyUsersSelect = $editModal.find('#user_id_any');
            anyUsersSelect.empty();
            // Preselect currently assigned users into both selects appropriately
            var selectedTaskUserIds = [];
            if (response.task && response.task.users && response.task.users.length > 0) {
                selectedTaskUserIds = response.task.users.map(function (u) { return u.id; });
                // For workspace select (ajax-based), append selected options so they are visible
                response.task.users.forEach(function (user) {
                    var opt = new Option(user.first_name + ' ' + user.last_name, user.id, true, true);
                    anyUsersSelect.append(opt);
                });
                // For project users select, select those that are present in its options
                var presentInProject = projectUsersSelect.find('option').map(function(){return parseInt($(this).val());}).get();
                var toSelectInProject = selectedTaskUserIds.filter(function(id){ return presentInProject.indexOf(id) !== -1; });
                projectUsersSelect.val(toSelectInProject).trigger('change');
            } else {
                projectUsersSelect.val(null).trigger('change');
                anyUsersSelect.val(null).trigger('change');
            }
            // Ensure toggle reflects current checkbox state after population
            toggleProjectLinkedUI($editModal);
...
function initSelect2WithAjax(selector, type) {
    $(selector).each(function () {
        if ($(this).length) {
            ...
            var ajaxOptions = {
                ajax: {
                    url: (typeof baseUrl !== 'undefined' ? baseUrl : '') + "/search", // API endpoint to fetch data dynamically
                    dataType: "json",
                    delay: 250,
                    data: function (params) {
                        return {
            ...
            //     };
            // }
            // If select already has options, fallback to local Select2 (no AJAX)
            if ($this.find('option').length > 0) {
                var localOptions = {
                    minimumInputLength: 0,
                    allowClear: allowClear,
                    closeOnSelect: singleSelect,
                    language: ajaxOptions.language,
                };
                // Always set dropdownParent when inside a modal to avoid z-index issues
                if ($this.closest('.modal').length) {
                    var modalIdLocal = $this.closest('.modal').attr('id');
                    if (modalIdLocal) {
                        localOptions.dropdownParent = $('#' + modalIdLocal);
                    }
                }
                $this.select2(localOptions);
                $(".cancel-button").on("click", function () {
                    $this.select2("close"); // Close the dropdown
                });
                return; // Skip AJAX init
            }

            // Check if the element is inside a modal; set dropdownParent regardless of single-select
            if ($this.closest(".modal").length) {
