        ...
        if ($type) {
            ...
            switch ($type) {
                ...
                case 'users':
                    ...
                    // Apply name filters on User model
                    $queryBuilder->where(function ($subQuery) use ($query) {
                        $subQuery->where('first_name', 'like', '%' . $query . '%')
                            ->orWhere('last_name', 'like', '%' . $query . '%')
                            ->orWhere(DB::raw("CONCAT(first_name, ' ', last_name)"), 'like', '%' . $query . '%');
                    });
                    // Scope by workspace if requested
                    if ($considerWorkspace == "true") {
                        $queryBuilder->whereHas('workspaces', function ($q) use ($workspace_id) {
                ...
                case 'clients':
                    ...
                    // Apply company/name filters on Client model
                    $queryBuilder->where(function ($subQuery) use ($query) {
                        $subQuery->where('company', 'like', '%' . $query . '%')
                    ...
                    foreach ($clients as $client) {
                        $display = $client->company ?: trim(($client->first_name . ' ' . $client->last_name));
                        $results[] = [
                            'id' => $client->id,
                            'text' => $display
                        ];
