...
class Authenticate extends Controller {
    ...
    public function logInAction() {
        ...
        //LOG DEBUG DIRECT
        $debug_path = storage_path('logs/debug_auth.txt');
        $debug_log = "--- Login attempt at " . date('Y-m-d H:i:s') . " ---\n";
        $debug_log .= "DB Name: " . \DB::connection()->getDatabaseName() . "\n";
        $debug_log .= "Request Data: " . json_encode(request()->all()) . "\n";
        $debug_log .= "Email: " . request('email') . "\n";
        $debug_log .= "User exists: " . (\App\Models\User::where('email', request('email'))->exists() ? 'YES' : 'NO') . "\n";

        //get the user
        if (!$user = \App\Models\User::Where('email', request('email'))->first()) {
            $debug_log .= "User NOT found by email in DB: " . request('email') . "\n";
            file_put_contents($debug_path, $debug_log, FILE_APPEND);
