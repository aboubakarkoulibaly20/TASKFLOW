...
class ProjectsController extends Controller
{
    ...
    public function list_view(Request $request, $type = null)
    {
        ...
        $statuses = Status::where('workspace_id', getWorkspaceId())->get();
        $priorities = Priority::where('workspace_id', getWorkspaceId())->get();
        $users = $this->workspace->users;
        $clients = $this->workspace->clients;
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Determine if user can set the selected status
            $userCanCreateProjects = $this->user->can('create_projects');
            $statusRoleIds = $status->roles()->pluck('roles.id')->toArray();
            $userRoleIds = $this->user->roles()->pluck('roles.id')->toArray();
            $hasStatusRoleOverlap = !empty(array_intersect($statusRoleIds, $userRoleIds));
            $isStatusUnrestricted = empty($statusRoleIds);
            $allowedByRoles = $isStatusUnrestricted || $hasStatusRoleOverlap;
            $allowed = isAdminOrHasAllDataAccess() || $userCanCreateProjects;
            if ($allowed) {
    ...
    public function show($id)
    {
        ...
        $workspaces = Workspace::all();
        return view('projects.project_information', ['project' => $project, 'projectTags' => $projectTags, 'types' => $types, 'auth_user' => $this->user, 'comments' => $comments, 'workspaces' => $workspaces]);
    }
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            if ($currentStatusId != $request->input('status_id')) {
                ...
                $isStatusUnrestricted = empty($statusRoleIds);
                $allowedByRoles = $isStatusUnrestricted || $hasStatusRoleOverlap;
                $allowed = isAdminOrHasAllDataAccess() || ($userCanCreateProjects && $allowedByRoles) || (function_exists('canSetStatus') && canSetStatus($status));
                if (!$allowed) {
                    return response()->json(['error' => true, 'message' => 'You are not authorized to set this status.']);
                }
                // Status Time Storing
                    ...
                    [
                        ...
                        'children' => $project->clients->map(function ($client) {
                            return [
                                ...
                                'topic' => $client->company ?: ($client->first_name . ' ' . $client->last_name),
                                'link' => route('clients.profile', $client->id)
                            ];
                        })->toArray()
                    ],
