...
class TasksController extends Controller
{
    ...
    public function index(Request $request, $id = '')
    {
        ...
        return view('tasks.tasks', ['project' => $project, 'tasks' => $tasks, 'is_favorites' => $is_favorites, 'workspaces' => $workspaces, 'users' => $users]);
    }
    /**
     * Create a new task.
     *
    ...
    {
        \Log::info('Sanitized inputs: ', $request->all());
        ...
        try {
            ...
            if (canSetStatus($status)) {
                ...
                $formFields['project_id'] = $project_id ?: null;
                // Merge users from both selectors: project-scoped (user_id[]) and workspace-wide (users_id[])
                $userIdsProject = (array) $request->input('user_id', []);
                $userIdsWorkspace = (array) $request->input('users_id', []);
                $userIds = array_values(array_unique(array_map('intval', array_merge($userIdsProject, $userIdsWorkspace))));
                unset($formFields['user_id']);
                // Also remove users_id if present to avoid mass assignment issues
                if (isset($formFields['users_id'])) {
                    unset($formFields['users_id']);
                }
                ...
                return formatApiResponse(
                    ...
                    [
                        'id' => $new_task->id,
                        'parent_id' => ($project_id ?: getWorkspaceId()), 'parent_type' => ($project_id ? 'project' : 'workspace'),
    ...
    public function show($id)
    {
        ...
        $workspaces = Workspace::all();
        return view('tasks.task_information', ['task' => $task, 'auth_user' => $this->user,'project' => $project, 'workspaces' => $workspaces ]);
    }
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $currentClients = $task->project ? $task->project->clients->pluck('id')->toArray() : [];
    ...
    public function list(Request $request, $id = '')
    {
        ...
        // Ensure $tasks is a query builder (not a plain Collection) before joins/pagination
        if ($tasks instanceof \Illuminate\Support\Collection) {
            $taskIds = $tasks->pluck('id')->all();
            $tasks = \App\Models\Task::query()->whereIn('tasks.id', $taskIds);
        }
        if (!empty($user_ids)) {
        ...
        $tasks = $tasks->leftJoin('pinned', function ($join) {
            ...
            ->through(function ($task) use ($statuses, $priorities, $canEdit, $canDelete, $canCreate, $isHome, $webGuard, $canManageProjects) {
                ...
                $clientHtml = '';
                if ($task->project && !empty($task->project->clients) && count($task->project->clients) > 0) {
                ...
                $isFavoriteProject = $task->project ? getFavoriteStatus($task->project->id) : false;
    ...
    public function dragula(Request $request, $id = '')
    {
        ...
        // Normalize to Query Builder if a Collection was returned from relation helpers
        if ($tasksQuery instanceof \Illuminate\Support\Collection) {
            $taskIds = $tasksQuery->pluck('id')->all();
            $tasksQuery = \App\Models\Task::query()->whereIn('tasks.id', $taskIds);
        }
        ...
        return view('tasks.board_view', ['project' => $project, 'tasks' => $tasks, 'total_tasks' => $total_tasks, 'is_favorites' => $isFavorites, 'workspaces' => $workspaces]);
    }
    public function updateStatus($id, $newStatus)
    {
        ...
        if (canSetStatus($status)) {
            ...
            if ($task->save()) {
                ...
                $clientIds = $task->project ? $task->project->clients->pluck('id')->toArray() : [];
                $recipients = array_merge(
                    array_map(function ($userId) {
                        return 'u_' . $userId;
                    }, $userIds),
    ...
    public function calendar(Request $request, $id = null)
    {
        ...
        $workspaces = Workspace::All();
        return view('tasks.calendar_view', ['project' => $project, 'is_favorites' => $isFavorites,'workspaces'=>$workspaces]);
    }
    ...
    public function group_by_task_list(Request $request)
    {
        try {
            ...
            $workspaces = Workspace::All();
            return view('tasks.group_by_task_lists', compact('taskLists', 'projects', 'tasks', 'toSelectTaskUsers','is_favorites','workspaces'));
        } catch (\Exception $e) {
            // dd($e);
