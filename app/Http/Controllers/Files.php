...
class Files extends Controller {
    ...
    public function renameFile($id) {
        ...
        //rename file
        try {
            $old_file = base_path() . '/storage/files/' . $file->file_directory . '/' . $file->file_filename;
            $new_file = base_path() . '/storage/files/' . $file->file_directory . '/' . $new_filename;
    ...
    public function bulkDownload() {
        ...
        //create zip archive
        $zip = new \ZipArchive();
        $zip_file = base_path() . "/storage/temp/$temp_directory/files.zip";
        ...
        foreach (request('ids') as $unique_id => $value) {
            if ($value == 'on') {
                ...
                if ($file = \App\Models\File::Where('file_uniqueid', $unique_id)->first()) {
                    ...
                    //file path
                    $file_path = base_path() . "/storage/files/" . $file->file_directory . "/" . $file->file_filename;
                    ...
                    if (file_exists($file_path)) {
                        ...
                        for ($i = 1; $i < 200; $i++) {
                            ...
                            if (file_exists(base_path() . "/storage/temp/$temp_directory/$target_file_name")) {
                        ...
                        $zip->addFile(base_path() . "/storage/temp/$temp_directory/$target_file_name", $target_file_name);
                    }
                }
            }
        }
            ...
            if ($value == 'on') {
                if ($project = \App\Models\Project::Where('project_id', request('copy_target_project'))->first()) {
                    if ($file = \App\Models\File::Where('file_uniqueid', $file_uniqueid)->first()) {
                        ...
                        //paths
                        $source = base_path() . "/storage/files/" . $file->file_directory;
                        $destination = base_path() . "/storage/files/$directory";
