...
function middlewareBootSettings() {
    ...
    if (false) { //env('SETUP_STATUS') != 'COMPLETED') {
        //skip this provider
        return;
    }
    ...
    //set timezone (ensure it's an object property access)
    $timezone = $settings->settings_system_timezone ?? 'UTC';
    ...
    try {
        foreach (\App\Models\LeadStatus::get() as $status) {
            ...
            $settings['lead_statuses'] += [$id => $color];
            $settings['lead_custom_statuses'] += [$id => $title];
        }
    } catch (\Exception $e) {}
    ...
    try {
        foreach (\App\Models\TaskStatus::get() as $status) {
            ...
            $settings['task_statuses'] += [$id => $color];
            $settings['task_custom_statuses'] += [$id => $title];
        }
    } catch (\Exception $e) {}
    ...
    try {
        foreach (\App\Models\TicketStatus::get() as $status) {
            ...
            $settings['ticket_statuses'] += [$id => $color];
            $settings['ticket_custom_statuses'] += [$id => $title];
        }
    } catch (\Exception $e) {}

    //Just a list of all payment geteways - used in dropdowns and filters
    $settings['gateways'] = ['Paypal', 'Stripe', 'Bank', 'Cash'];

    //cronjob path
    $settings['cronjob_path'] = '/usr/local/bin/php ' . base_path() . '/artisan schedule:run >> /dev/null 2>&1';

    //all team members
    try {
        $settings['team_members'] = \App\Models\User::Where('type', 'team')->Where('status', 'active')->get();
    } catch (\Exception $e) { $settings['team_members'] = collect([]); }

    //the main admin user
    try {
        $settings['main_admin_user'] = \App\Models\User::Where('id', 1)->first();
    } catch (\Exception $e) { $settings['main_admin_user'] = null; }

    //javascript file versioning to avoid caching when making updates
    $settings['versioning'] = $settings->settings_system_javascript_versioning ?? '1';
...
function middlewareBootTheme() {
    ...
    $directories = glob(base_path('public/themes/*'), GLOB_ONLYDIR);
    ...
    //check if theme name is not set or is invalid - set to default
    if (empty($settings->settings_theme_name) || !in_array($settings->settings_theme_name, $directories)) {
        //if theme name is not valid, reset to default
        if (!in_array('default', $directories)) {
            Log::critical("No valid theme directory could be found", ['process' => '[validating theme]', config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            abort(409, __('lang.default_theme_not_found') . ' [Default]');
        }
        //set to default theme
        $settings->settings_theme_name = 'default';
        $settings->save();
    }
    ...
    if (is_file(base_path("public/themes/$active_theme/css/style.css"))) {
