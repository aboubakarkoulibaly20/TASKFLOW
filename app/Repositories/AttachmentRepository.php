...
class AttachmentRepository {
    ...
    public function process($data = []) {
        ...
        $file_path = base_path() . "/storage/temp/" . $data["attachment_directory"] . "/" . $data["attachment_filename"];
        $old_dir_path = base_path() . "/storage/temp/" . $data["attachment_directory"];
        $new_dir_path = base_path() . "/storage/files/" . $data["attachment_directory"];
    ...
    public function processAvatar($data = []) {
        ...
        //path to this directory in the temp folder
        $file_path = base_path() . "/storage/temp/" . $data["directory"] . "/" . $data["filename"];
        $old_dir_path = base_path() . "/storage/temp/" . $data["directory"];
        $new_dir_path = base_path() . "/storage/avatars/" . $data["directory"];
    ...
    public function processClientLogo($data = []) {
        ...
        $file_path = base_path() . "/storage/temp/" . $data["directory"] . "/" . $data["filename"];
        $old_dir_path = base_path() . "/storage/temp/" . $data["directory"];
        $new_dir_path = base_path() . "/storage/logos/clients/" . $data["directory"];
    ...
    public function processAppLogo($data = []) {
        ...
        //path to this directory in the temp folder
        $uploaded_file = base_path() . "/storage/temp/" . $data["directory"] . "/" . $data["filename"];
        $new_file = base_path() . "/storage/logos/app/" . $data["new_filename"];
    ...
    /**
     *   Process any files that needs to just be moved from temp to files directory
     *   1. Check that the folder and file exist in the 'temp' directory
     *   2. move the whole directory in the the 'files' directory
     * @param array $data payload data
     * @return array new file data
     */
    public function processGeneralFile($data = []) {

        //validate
        if(!isset($data["directory"]) || $data["directory"] == ''){
            Log::error("file data is missing", ['process' => '[AttachmentRepository]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__]);
            return false;
        }
        ...
        $file_path = base_path() . "/storage/temp/" . $data["directory"] . "/" . $data["filename"];
        $old_dir_path = base_path() . "/storage/temp/" . $data["directory"];
        $new_dir_path = base_path() . "/storage/files/" . $data["directory"];
        $new_file_path = base_path() . "/storage/files/" . $data["directory"] . "/" . $data["filename"];
        $extension = pathinfo($file_path, PATHINFO_EXTENSION);
        $filesize = humanFileSize(filesize($file_path));
