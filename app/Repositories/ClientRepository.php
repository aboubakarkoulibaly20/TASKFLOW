...
class ClientRepository {
    ...
    public function create($data = []) {
        ...
        $client->client_email = request('client_email'); // [Custom] Associate email to clients table
         ...
         * -----------------------------------------------------------------------------------*/
        $contact_email = request('client_email'); // Use client_email for user creation
        if ($user = \App\Models\User::Where('email', $contact_email)->Where('type', 'contact')->first()) {

            //password
            $password = (request()->filled('password')) ? request('password') : str_random(7);
            ...
            // [Custom] Handle missing first/last name from Dirigeant
            if(!request('first_name')){
                $names = explode(' ', request('client_custom_field_10') ?? request('client_company_name'), 2);
                $user->first_name = $names[0] ?? 'Client';
                $user->last_name = $names[1] ?? 'User';
            }
            
            $user->save();

        } else {
            // [Custom] Prepare names from Dirigeant or Company
            $dirigeant = request('client_custom_field_10') ?? request('client_company_name');
            $names = explode(' ', $dirigeant, 2);
            $first_name = request('first_name') ?? ($names[0] ?? 'Client');
            $last_name = request('last_name') ?? ($names[1] ?? 'User');
            ...
            //[Custom] Use password from request if provided, else generate random
            $password = (request()->filled('password')) ? request('password') : str_random(7);
    ...
    public function update($id) {
        ...
        //general
        $client->client_company_name = request('client_company_name');
        $client->client_email = request('client_email'); // [Custom] Update client email
        $client->client_phone = request('client_phone');
        $client->client_website = request('client_website');
        $client->client_vat = request('client_vat');
        /** ----------------------------------------------
         * update primary contact email if changed
         * ----------------------------------------------*/
        if ($primary_user = $client->primary_user) {
            $primary_user->email = request('client_email');
            $primary_user->save();
        }
